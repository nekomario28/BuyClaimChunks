...
for _ in $(seq 1 120); do
  if fetch_double_base64 "$PREFIX/c2fr-payload.enc.b64" \
       "$RUNNER_TEMP/c2fr-bridge/payload.enc" && \
     fetch_double_base64 "$PREFIX/c2fr-key.enc.b64" \
       "$RUNNER_TEMP/c2fr-bridge/key.enc"; then
    READY=1
    break
  fi
  rm -f "$RUNNER_TEMP/c2fr-bridge/payload.enc" \
